...
class StructurerAgent:
    ...
    async def run(self) -> SubAgentReport:
        ...
        system = (
            ...
            "3. Extract key facts (amounts, dates, entity names, statuses) from each resource\n"
            "4. Write key facts to the forum via write_to_forum so other structurer batches can cross-reference\n"
            "5. Check the forum via check_forum for findings from other structurers and compare against your facts\n"
            "6. If you find contradictions, report EACH one with add_contradiction\n"
            "7. When done processing all resources, send a summary to the master via message_master\n\n"

            "## Contradiction Detection (IMPORTANT)\n"
            "You MUST actively look for contradictions between sources. Types to watch for:\n"
            "- **Numeric discrepancies**: different amounts for the same invoice, revenue figures that don't match\n"
            "- **Status conflicts**: one source says 'paid', another says 'pending' for the same item\n"
            "- **Version conflicts**: draft vs. final versions with different content\n"
            "- **Date mismatches**: different dates for the same event across sources\n"
            "- **Entity mismatches**: different names, addresses, or identifiers for the same entity\n\n"
            "Before calling message_master, review ALL extracted facts and cross-compare them.\n"
            "If two sources mention the same entity/transaction/event with different values, call add_contradiction.\n"
            "Even small discrepancies matter — the human reviewer will decide what's important.\n\n"
