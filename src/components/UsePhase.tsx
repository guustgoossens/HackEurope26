...
import { MessageSquare, BookOpen, Layers, Folder } from 'lucide-react';
...
export default function UsePhase({ clientId: _clientId, onBack }: Props) {
  ...
  -d '{"question": "Latest Acme invoice?"}'`;
    ...
    return (
        <div className="flex flex-col h-full w-full overflow-hidden" style={{ animation: 'fade-in 0.4s ease-out', background: 'hsl(0 0% 100%)' }}>
            {/* Dashboard Header */}
            <header className="h-[72px] flex items-center justify-between px-8 shrink-0 border-b border-slate-200 bg-white" style={{ zIndex: 10 }}>
                <div className="flex items-center gap-6">
                    <button onClick={onBack} className="flex items-center gap-3 hover:opacity-80 transition" aria-label="Back">
                        <img src={folioMark} alt="Folio" className="w-[48px] h-[48px]" />
                ...
                <div className="flex items-center gap-5">
                    ...
                    <div className="flex bg-slate-100 p-1 rounded-xl">
                        <button
                            ...
                            className={cn('flex items-center gap-2 px-5 py-2 text-sm font-medium rounded-lg transition-colors', activeTab === 'docs' ? 'bg-white shadow-sm text-foreground' : 'text-slate-500 hover:text-slate-700')}
                        >
                        ...
                        <button
                            ...
                            className={cn('flex items-center gap-2 px-5 py-2 text-sm font-medium rounded-lg transition-colors', activeTab === 'api' ? 'bg-white shadow-sm text-foreground' : 'text-slate-500 hover:text-slate-700')}
                        >
                            <FolioCode className="w-4 h-4" />
                            API & Integrations
                        </button>
                    </div>
                    ...
                    <button
                        ...
                        className="btn-organic flex items-center gap-2 px-5 py-2.5 text-white text-sm font-medium rounded-xl transition-all"
                    ...
                    <div className="flex items-center gap-3">
                        <span className="text-sm font-medium text-slate-600">Cabinet Dupont & Associés</span>
            ...
            <div className="flex flex-1 overflow-hidden w-full relative bg-slate-50/50">
                {/* ── Main Area ── */}
                <div className="flex-1 flex flex-col min-w-0 overflow-y-auto">
                    {activeTab === 'docs' ? (
                        <div className="flex flex-col h-full max-w-7xl mx-auto w-full p-8">
                            <div className="flex items-center justify-between mb-8">
                                <h2 className="text-3xl font-medium text-slate-800" style={{ fontFamily: "'Newsreader', serif" }}>{t('use.kbTitle')}</h2>
                                <div className="flex bg-slate-200/50 p-1.5 rounded-xl">
                                    <button
                                        onClick={() => setViewMode('tree')}
                                        className={cn('px-6 py-2 rounded-lg text-sm font-medium transition-all', viewMode === 'tree' ? 'bg-white shadow-sm text-slate-800' : 'text-slate-500 hover:text-slate-700')}
                                    >
                                        Folder Tree
                                    </button>
                                    <button
                                        onClick={() => setViewMode('graph')}
                                        className={cn('px-6 py-2 rounded-lg text-sm font-medium transition-all', viewMode === 'graph' ? 'bg-white shadow-sm text-slate-800' : 'text-slate-500 hover:text-slate-700')}
                                    >
                                        Graph View
                                    </button>
                                </div>
                            ...
                            <div className="flex-1 bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden flex flex-col">
                                {viewMode === 'graph' ? (
                                    <div className="w-full h-full min-h-[600px] bg-slate-900">
                                        <VisualizationGraph clientId={_clientId as Id<'clients'>} type="knowledge" />
                                    </div>
                                ) : (
                                    <div className="flex h-full min-h-[600px] flex-col lg:flex-row">
                                        <div className="w-full lg:w-1/3 shrink-0 flex flex-col border-r border-slate-200 bg-slate-50/30">
                                            <div className="p-6 border-b border-slate-200">
                                                <input
                                                    ...
                                                    className="w-full text-base px-5 py-3.5 rounded-xl border border-slate-300 bg-white shadow-sm focus:outline-none focus:border-slate-400 focus:ring-1 focus:ring-slate-200 transition-shadow"
                                                />
                                            </div>
                                            <div className="flex-1 overflow-y-auto p-4 space-y-4">
                                                    ...
                                                    return (
                                                        <div key={domain.id} className="space-y-1">
                                                            <button
                                                                ...
                                                                className={cn(
                                                                    'w-full flex items-center gap-4 px-4 py-3 text-lg font-medium rounded-xl text-left transition-colors duration-150',
                                                                    selectedFolder === domain.id
                                                                        ? 'text-primary bg-primary/10'
                                                                        : 'text-slate-700 hover:bg-slate-100'
                                                                )}
                                                            >
                                                                <DomainIcon className="h-6 w-6 shrink-0 text-primary" />
                                                            ...
                                                            {domain.children?.map(child => {
                                                                ...
                                                                return (
                                                                    <button
                                                                        ...
                                                                        className={cn(
                                                                            'w-full flex items-center gap-3 pl-12 pr-4 py-2.5 text-base rounded-lg text-left transition-colors duration-150',
                                                                            selectedFolder === child.id
                                                                                ? 'text-primary font-medium bg-primary/10'
                                                                                : 'text-slate-600 hover:bg-slate-100'
                                                                        )}
                                                                    >
                                                                        <ChildIcon className="h-5 w-5 shrink-0 opacity-70" />
                                                ...
                                                {filteredTree.length === 0 && (
                                                    <p className="text-base text-center text-muted-foreground mt-8 italic">
                                                        No match
                                                    </p>
                                                )}
                                            </div>
                                        </div>
                                        <div className="w-full lg:w-2/3 flex flex-col bg-white p-12 items-center justify-center text-center relative overflow-hidden">
                                            <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,theme(colors.slate.50)_0%,transparent_100%)] opacity-50" />
                                            {selectedFolder ? (
                                                <div className="space-y-6 max-w-md relative z-10">
                                                    <div className="h-28 w-28 mx-auto bg-slate-50 border border-slate-100 rounded-[2rem] shadow-sm flex items-center justify-center">
                                                        <Folder className="h-12 w-12 text-slate-300" />
                                                    </div>
                                                    <div>
                                                        <h3 className="text-2xl font-medium text-slate-800 mb-3">Dossier sélectionné</h3>
                                                        <p className="text-base text-slate-500 leading-relaxed">Le contenu détaillé des documents peut être exploré et synthétisé dans le chat contextuel.</p>
                                                    </div>
                                                </div>
                                            ) : (
                                                <div className="space-y-6 max-w-md opacity-60 relative z-10">
                                                    <Layers className="h-20 w-20 mx-auto text-slate-300" />
                                                    <p className="text-xl text-slate-500 font-medium">Parcourez l'arborescence pour explorer les connaissances</p>
                                                </div>
                                            )}
                                        </div>
                                    </div>
                        ...
                        <div
                            className="w-full max-w-4xl mx-auto flex flex-col py-8 px-4 gap-6"
                        >
                            {/* ── Right: Tech Stack / API & Integrations ─────────────────────── */}
                            {/* Agents Tracking */}
                            <div className="px-8 py-6 bg-white rounded-2xl border border-slate-200 shadow-sm">
  ...
  "sourceCount": 2
                ...
                </div>                {/* Sliding Chat Panel */}
                {showChat && (
                    <div className="w-[480px] shrink-0 border-l border-slate-200 bg-white shadow-xl z-20 flex flex-col" style={{ animation: 'slide-left 0.3s ease-out' }}>
